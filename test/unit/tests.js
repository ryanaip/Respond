...
window.onload = function(){
	...
	if( !window.opener ){
		...
		function colorApplied( val ){
		  return testElem.currentStyle.backgroundColor == val;
		}
		...
		asyncTest( "inline styles with a media query apply properly", function() {
		  window.resizeTo(1400,600);
		  setTimeout(function(){
		    ok( colorApplied( 'red' ), "testelem is 30px wide when window is > 1400px wide" );
		    start();
		  }, 900);
		});
	}
